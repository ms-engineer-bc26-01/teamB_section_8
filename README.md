...
---
###  Backend
- **FastAPI** : 軽量で高速なAPIフレームワーク。
- **REST** : APIの構築に使用。
- **Python** : バックエンドの主要言語。
- **SQLAlchemy** : データベース操作を行うORM。
- **Pydantic** : リクエスト・レスポンスのデータバリデーション。
- **Uvicorn** : ASGIサーバー（開発用）
### AI / LLM
- **OpenAI API** : ユーザー入力をもとにコーディネート提案を生成。
- **カスタムプロンプト設計**
-- stylist_prompt.py：スタイリスト用プロンプト定義
-- llm.py：LLM呼び出しロジック
### Frontend
- **Next.js** : Reactベースのフレームワーク。SSR/CSR対応。
- **React** : UIコンポーネント構築。
- **Axios** : バックエンドAPIとの通信。
### Infrastructure / External Services
- **Firebase** : 認証および一部バックエンドサービス。
- **OpenWeather API** : 天気情報の取得（郵便番号ベース）。
### Development Tools
...
[ FastAPI (Backend API) ]
 ...
 ┌───────────────┬───────────────┐
 │               │               │
[ OpenAI API ] [ Weather API ] [ Firebase ]

- フロントエンドとバックエンドを分離した構成
- 外部API（AI / 天気）を組み合わせた機能提供
- LLMをビジネスロジックに組み込んだ設計
